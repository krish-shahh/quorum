...
class ExecutionLog:
    ...
    def _get_db(self):
        ...
        if "db_path" not in cfg:
            # log_path is typically ~/.tradingagents/execution/trades.jsonl
            # so parent.parent gives ~/.tradingagents/
            cfg = dict(cfg, db_path=str(self._log_path.parent.parent / "tradingagents.db"))
        return get_db(cfg)
    ...
    def query_trades(
        ...
        *,
        ...
        """
        try:
            from tradingagents.execution.db import query_trades
            return query_trades(
