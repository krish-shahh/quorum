...
Requires a one-time OAuth2 browser auth via ``quorum auth-schwab``
...
class SchwabBrokerClient(BrokerClient):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._token_path = str(
            Path(config.get("schwab_token_path", "~/.quorum/schwab_token.json")).expanduser()
        )
        ...
        if not raw_hashes:
            raise ValueError(
                ...
                "Run 'quorum auth-schwab' to obtain it."
            )
