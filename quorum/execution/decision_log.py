...
def new_run(
    ...
    *,
    ...
    Reads QUORUM_CYCLE_ID from the environment (set once by the `quorum
    cycle` CLI wrapper before spawning `claude -p`) so every run created
    during that invocation — pod-cycle creates a fresh run per
    get_pod_candidates/get_pod_exits call — correlates back to one trace.
    No parameter-threading needed: the env var inherits down the whole
    `claude -p` -> MCP server -> tool-handler process tree.
    """
    run_id = _new_id()
    cycle_id = os.environ.get("QUORUM_CYCLE_ID")
...
def list_runs(
    ...
    rows = conn.execute(
        ...
        f"started_at, finished_at, gate_passed, metrics_json, cycle_id FROM run "
...
def get_run_detail(config: Dict[str, Any], run_id: str) -> Optional[Dict[str, Any]]:
    ...
    run = conn.execute(
        ...
        "finished_at, error, gate_passed, gate_result_json, metrics_json, cycle_id FROM run "
        "WHERE run_id = ?", (run_id,),
    ).fetchone()
    if run is None:
        return None
...
# ── trace_event / cycle ──────────────────────────────────────────────
#
# One `quorum cycle` invocation (one `claude -p "/pod-cycle"` spawn) gets
# one cycle_id, generated by the CLI and inherited via QUORUM_CYCLE_ID down
# to every `new_run()` call the invocation makes (see new_run() above).
# record_trace_event() is called once per parsed stream-json line by the
# CLI's own parser — this module has no subprocess-spawning code itself,
# it just persists whatever the CLI hands it.
    ...
    *,
    ...
    with conn:
        conn.execute(
            ...
            "(event_id, cycle_id, session_id, parent_tool_use_id, role, event_type, "
            " tool_name, tool_input_json, tool_output_summary, text) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                event_id, cycle_id, session_id, parent_tool_use_id, role, event_type,
                tool_name, json.dumps(tool_input, default=str) if tool_input is not None else None,
...
def get_cycle(config: Dict[str, Any], cycle_id: str) -> Optional[Dict[str, Any]]:
    """Full ordered trace for one cycle, plus which runs it touched — the
    drill-in behind a Logs view. None if no trace_event rows exist for
    this cycle_id (e.g. a run created outside `quorum cycle`)."""
    conn = db.get_db(config)
    events = conn.execute(
        "SELECT event_id, ts, session_id, parent_tool_use_id, role, event_type, "
        "tool_name, tool_input_json, tool_output_summary, text FROM trace_event "
        "WHERE cycle_id = ? ORDER BY ts ASC, rowid ASC", (cycle_id,),
    ).fetchall()
    if not events:
        return None

    runs = conn.execute(
        "SELECT run_id, strategy_id, mode, status FROM run WHERE cycle_id = ? "
        "ORDER BY started_at ASC", (cycle_id,),
    ).fetchall()
    ...
    return {
        ...
        "trace_events": [
            {**{k: row[k] for k in row.keys() if k != "tool_input_json"},
...
def list_cycles(config: Dict[str, Any], limit: int = 50) -> List[Dict[str, Any]]:
    """List recent cycles newest-first — one row per distinct cycle_id
    seen in trace_event, with a cheap event count and the run_ids/status
    it touched, for a Logs view picker."""
    conn = db.get_db(config)
    cycles = conn.execute(
        "SELECT cycle_id, MIN(ts) AS started_at, COUNT(*) AS n_trace_events "
        "FROM trace_event GROUP BY cycle_id ORDER BY started_at DESC LIMIT ?",
        (limit,),
    ).fetchall()
    ...
    for c in cycles:
        runs = conn.execute(
            "SELECT run_id, status FROM run WHERE cycle_id = ?", (c["cycle_id"],),
        ).fetchall()
        statuses = {r["status"] for r in runs}
        if "running" in statuses:
            status = "running"
        elif "error" in statuses or "killed" in statuses:
            status = "error"
        elif statuses:
            status = "ok"
        else:
        ...
        result.append({
            ...
            "n_trace_events": c["n_trace_events"],
