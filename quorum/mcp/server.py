...
def _handle_tool(name: str, args: dict) -> str:
    ...
    if name == "get_kalshi_markets":
        from quorum.dataflows.kalshi import get_markets
        markets = get_markets(
    ...
    if name == "get_kalshi_market":
        from quorum.dataflows.kalshi import get_market
    ...
    if name == "get_kalshi_orderbook":
        from quorum.dataflows.kalshi import get_orderbook
    ...
    if name == "get_kalshi_events":
        from quorum.dataflows.kalshi import get_events
        events = get_events(
            limit=int(args.get("limit", 20)),
            series_ticker=args.get("series_ticker"),
    ...
    if name == "get_kalshi_event":
        from quorum.dataflows.kalshi import get_event
    ...
    if name == "scan_kalshi_overround":
        from quorum.dataflows.arb_scanner import scan_overround
        from quorum.execution.db import get_db
    ...
    if name == "scan_kalshi_bias":
        from quorum.dataflows.arb_scanner import scan_bias, BIAS_BUCKETS
        from quorum.execution.db import get_db
        import json as _json
    ...
    if name == "get_dutch_book_detail":
        from quorum.dataflows.arb_scanner import calculate_dutch_book
    ...
    if name == "get_prediction_candidates":
        from quorum.dataflows.arb_scanner import get_council_candidates
