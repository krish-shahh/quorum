"""quorum health check — validates the full stack before trading.

Usage:
    quorum health          # CLI command
    python -m quorum.mcp.health  # standalone
"""
...
def check_mcp_package() -> CheckResult:
    try:
        ...
        return ("MCP package installed", False, "pip install 'quorum[mcp]'")
...
def check_kill_switch() -> CheckResult:
    ...
    try:
        ...
        if active:
            return ("Kill switch", False, "ACTIVE — trading is halted! Run: quorum reset-kill-switch")
...
def check_mcp_server_starts() -> CheckResult:
    ...
    try:
        t0 = time.time()
        from quorum.mcp.server import create_server
...
def check_mcp_stdio_protocol() -> CheckResult:
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "quorum.mcp.server"],
            input=payload,
            capture_output=True,
            text=True,
            timeout=15,
