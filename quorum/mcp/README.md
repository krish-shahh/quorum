...
| Tool | Description |
|------|-------------|
| `get_autonomous_tickers` | Get tickers from `~/.quorum/tickers.txt` + current portfolio state + market regime |
...
**One-time setup:** Edit `~/.quorum/tickers.txt` with your tickers:
```
AAPL
MSFT
NVDA
...
The dashboard (`quorum`) shows all trades, positions, and wiki pages in real time.
